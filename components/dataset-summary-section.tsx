...
import { DatasetDetails, ModelDetails, Organization } from "@/types"
...
const terminalScript = (modelName: string): string => {
  const cliCommand = `
  shaped view-dataset --dataset-name ${modelName}`
  return cliCommand
}
...
const curlScript = (apiKey: string, uri: string): string => {
  const curlCommand = `
  curl ${uri} \\
      -H "x-api-key: ${apiKey}" \\
...
const curlScriptMarkdown = (apiKey: string, uri: string): string => {
  const curlCommand = `
  ~~~bash
  curl ${uri} \\
        -H "x-api-key: ${apiKey}" \\
...
export function DatasetSummarySection({
  ...
  const [requestType, setRequestType] = React.useState("cURL")

  const getMarkdown = (requestType: string) => {
    const markdown =
      requestType == "cURL"
        ? curlScriptMarkdown(organization.apiKey, datasetDetails.dataset_uri)
        : terminalScriptMarkdown(datasetDetails.dataset_name)
    return markdown
  }
  ...
  const getMarkdownCopyContent = (requestType: string) => {
    const markdown =
      requestType == "cURL"
        ? curlScript(organization.apiKey, datasetDetails.dataset_uri)
        : terminalScript(datasetDetails.dataset_name)
    return markdown
  }
      ...
      >
            ...
            <Dialog.Content className="fixed left-[50%] top-[50%] z-50 max-h-[95vh] w-[60%]  translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] focus:outline-none data-[state=open]:animate-content-show">
              <div className="flex flex-col space-y-8">
                <div>
                  ...
                  <div>See request details for this dataset.</div>
                </div>
                ...
                <Outline className="p-2">
                  ...
                  <div className="flex p-2">
                    <div className="flex w-[10rem] space-x-2 font-medium">
                      <div className="text-gray-900 underline">Dataset URI</div>
                    </div>
                    <a
                      href={datasetDetails.dataset_uri}
                      className="text-gray-500 underline"
                    >
                      {datasetDetails.dataset_uri}
                    </a>
                  </div>
                </Outline>
        ...
        <div className="mt-5 flex w-[60%] justify-between">
          <div className="flex-col space-y-1">
            ...
            <StatusChip status={datasetDetails.status} />
          </div>

          <div className="flex-col space-y-1">
            <IconTitle title="Connector" iconName="cog" />
            <div className="text-grey-900 capitalize underline hover:cursor-pointer">
              {datasetDetails.schema_type.toLowerCase()}
            </div>
          </div>
          ...
          <div className="flex-col space-y-1">
            ...
            <div className="text-grey-900">
              {getTimeDiffFromDateString(datasetDetails?.created_at)}
            </div>
          </div>
        </div>
